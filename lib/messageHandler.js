...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        const viewOnceContainer = message.message?.viewOnceMessageV2 || message.message?.viewOnceMessage;
        if (viewOnceContainer) {
            try {
                await storeMessage(sock, message);
            } catch (e) {
                console.error('ViewOnce detection error:', e);
            }
        }
        ...
        for (const call of calls) {
            ...
            try {
                ...
                if (!antiCallNotified.has(callerJid)) {
                    ...
                    const anticallMsg = {
                        text: `*🚫 [ AUTO REJECT ] 🚫*\n\n` +
                              `*Infinity MD* has detected an incoming call.\n` +
                              `Calls are not allowed to maintain bot stability.\n\n` +
                              `*👤 Caller:* @${callerJid.split('@')[0]}\n` +
                              `*🕒 Time:* ${new Date().toLocaleString()}\n\n` +
                              `_Please use text messages instead._`,
                        mentions: [callerJid],
                        contextInfo: {
                            externalAdReply: {
                                title: "INFINITY MD - CALL PROTECTION",
                                body: "Auto-Rejection Active",
                    ...
                    await sock.sendMessage(callerJid, anticallMsg);
                    printLog('info', `Sent beautiful anticall warning to: ${callerJid.split('@')[0]}`);
                }
